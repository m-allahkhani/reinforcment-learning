...
def every_visit_mc_prediction(env, policy, num_episodes, gamma):
    # initialize value function V(s) and number of visits N(s)
    V = np.zeros(env.observation_space.n)
    N = np.zeros(env.observation_space.n)
    ...
    for i_episode in range(num_episodes):
        ...
        # update the value function using the generated episode
        g = 0
        visited_states = set()
...
if __name__ == "__main__":
    ...
    # V_MC = first_visit_mc_prediction(env, get_policy_direction("down", map), num_episodes, gamma)
